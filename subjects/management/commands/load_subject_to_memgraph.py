"""Load a subject's class-ancestor closure into Memgraph in one go.

One WDQS query gets the seed entity and its P31?/P279* ancestors (labels +
class edges only). The response is parsed into per-entity groups and
applied atomically to Memgraph in a single Bolt transaction.
...
from subjects.memgraph import GRAPH_ERRORS
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if seed_item is None:
            ...
            # just fetched. bulk_create bypasses save()'s closure fetch,
            ...
            # so no closure re-fetch is triggered.
            seed_item.save(update_fields=list(SEED_METADATA_FIELDS))

        try:
            new_count = commit_closure_to_memgraph(
                seed_qid,
                data["groups"],
                data["labels"],
            ...
            raise CommandError(f"Memgraph update failed: {e}") from e
