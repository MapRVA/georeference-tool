...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total_images == 0:
            self.stdout.write(
                self.style.WARNING('No images found to process.')
            )
            return
    ...
    def setup_model(self, options):
        ...
        if local_model_path.exists():
            self.stdout.write(f'Loading CLIP model {model_name} from {local_model_path.parent}')
    ...
    def get_images_queryset(self, options):
        ...
        if options['image_ids']:
            try:
                ...
                raise CommandError('Invalid image IDs format. Use comma-separated integers.')
        ...
        for image in batch_images:
            try:
                ...
                if pil_image is None:
                    self.stdout.write(
                        self.style.WARNING(f'Failed to download image {image.id}: {image.permalink}')
                    )
                    continue
    ...
    def download_image(self, url: str, timeout: int = 30) -> Optional[PILImage.Image]:
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f'Failed to download {url}: {str(e)}')
            )
            return None
        except Exception as e:
            self.stdout.write(
