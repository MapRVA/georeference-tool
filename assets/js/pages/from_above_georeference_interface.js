...
document.addEventListener("DOMContentLoaded", function () {
    ...
    switchToOverlayLayer(layerId, tileUrl, title, tileType, attribution) {
      ...
      if (!this.map.getLayer(layerId)) {
        ...
        try {
          ...
          const beforeId =
            geomanLayerId ||
            (this.map.getLayer("polygon-fill") ? "polygon-fill" : undefined);
  ...
  if (isAuthenticated) {
    ...
    function addSubjectToDisplay(subject) {
      ...
      if (!subjectsRow) {
        const cardBody = document
          .querySelector("#add-subject-collapse")
          .closest(".card-body");
    ...
    function addSubjectByWikidataId(wikidataId) {
      ...
      fetch(config.urls.addSubjectToImage, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRFToken": csrfToken,
    ...
    document.addEventListener("click", function (e) {
      ...
      if (removeButton) {
        ...
        const csrfToken =
          document.querySelector('[name="csrfmiddlewaretoken"]')?.value ||
          config.csrfToken;
        const url = config.urls.removeSubjectFromImagePattern.replace(
          "/0/",
          `/${subjectRelationId}/`,
        );
